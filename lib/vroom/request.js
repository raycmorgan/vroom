...
exports.Request.prototype = {
   ...
   * @param {Boolean} opts.replace If true, it will remove all headers with the
   *                  key and set the given one.
   */
  addHeader: function (key, value, opts) {
    opts = opts || {};
    if (opts.replace)
      this.removeHeader(key);
    
    if (!this.hasSentHeader)
      this._header.push([key, value]);
  },
   ...
   * @param {Object} options Key/value pairs of cookie options
   */
  setCookie: function(name, value, options) {
    if(this.hasSentHeader)
      throw(Vroom.Exceptions.InternalServerError("Cannot set a new cookie once the headers have been sent."));
  ...
  deleteCookie: function(name) {
    if(this.hasSentHeader)
      throw(Vroom.Exceptions.InternalServerError("Cannot delete a cookie once the headers have been sent."));
        
    var cookie = name + "=null; expires=Thu, 2 Aug 2001 20:47:11 UTC"
    this.addHeader("Set-Cookie",cookie);
  }
};
